...
# 看是否在首頁, 是的話開始進行AI消除筆
if poco(text="AI消除笔").exists():
    poco(text="AI消除笔").click()
    ...
    sleep(1.0)
    if poco(text="消除").exists():
        poco(text="消除").click()
        poco(text="魔法消除").click()
    sleep(1.0)

    if poco(text="快速消除").exists():
        poco(name="com.photogrid.collage.videomaker:id/currentRemoveFuncIv").click()

    if poco(text="魔法消除").exists():
        poco(text="魔法消除").click()
...
# 等待目標字串出現
target_string = "com.photogrid.collage.videomaker:id/removeLoadingProgressBar"  # 將此處替換為你要等待的字串
...
if duration > one:
    ...
    sleep(5.0)
    # 重新执行步骤
    if poco(text="AI消除筆").exists():
        poco(text="AI消除筆").click()
        poco("android.widget.FrameLayout").child("android.widget.LinearLayout").offspring(
            "com.photogrid.collage.videomaker:id/page_root").offspring(
            "com.photogrid.collage.videomaker:id/selector_layout").offspring(
            "com.photogrid.collage.videomaker:id/grid_container").offspring(
    ...
    if poco(text="無網路連線").exists():
        ...
        send_message(message_body)
    # 等待目标字符串出现
    target_string = "com.photogrid.collage.videomaker:id/removeLoadingProgressBar"
    poco(name=target_string).wait_for_appearance()
...
#             send_message(message_body)
    ...
    if poco(text="網絡錯誤").exists():
        # 構建訊息
        message_body = 'AI消除筆(Super)第二次執行失敗\n錯誤提示:網路錯誤\n請檢查測試機網路'
        # 發送結果到飛書機器人群組
        send_message(message_body)
    if duration > two:
        print("第二次超過120秒")
    ...
    print(f"AI消除筆完成！回傳時間為：{duration:.2f}秒")
