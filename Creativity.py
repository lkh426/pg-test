...
#指定devices
connect_device("Android://127.0.0.1:5037/J3AXB762E8793AA")
# connect_device("Android://127.0.0.1:5037/9d397e81")#小米MIX2S
# 初始化Airtest
auto_setup(__file__)
...
if poco(text="編輯").exists():
    ...
    # 遍歷點擊指定的AI特效
    for i in range(20):
        if poco(text="Mystic Elf").exists():
            poco(text="Mystic Elf").click()
            break
        else:
            swipe((0.9*x, 0.9*y), (0.1*x, 0.9*y))
else:
...
if duration > two:
    ...
    if poco(text="編輯").exists():
        ...
        while poco(text=target_string).exists():
            elapsed_time = time.time() - start_time
            if elapsed_time >= 120 and not sent_60s_message:
#                 send_message('AI風格化第二次執行\n等待時間超過120秒')
                sent_60s_message = True
            if elapsed_time >= two:
                send_message('【加急加急加急】\nAI風格化(instant_id)第二次執行\n等待時間超過120秒,請QA手動測試目前狀況')
                stop_app(package=web)
                break
            time.sleep(1)
        ...
        if poco(text="網絡錯誤").exists():
            send_message('【加急加急加急】\nAI風格化(instant_id)第二次執行失敗\n錯誤提示:網路錯誤\n請檢查測試機網路')

else:
    print(f"AI風格化完成！回傳時間為：{duration:.2f}秒")
