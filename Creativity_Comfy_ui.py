...
if poco(text="編輯").exists():
    ...
    # 遍歷點擊指定的AI特效
    for i in range(20):
        if poco(text="Cartoon pet").exists():
            poco(text="Cartoon pet").click()
            break
        else:
            swipe((0.9*x, 0.9*y), (0.1*x, 0.9*y))
else:
...
if poco(text="無網路連線").exists():
    send_message('AI風格化失敗(Comfy_ui)\生成失敗\n請QA手動測試目前狀況')

if poco(text="網絡錯誤，請再試一次。").exists():
    send_message('AI風格化失敗(Comfy_ui)\n錯誤提示:網路錯誤\n請檢查測試機網路')
...
print("開始進行AI風格化(Comfy_ui_Cartoon pet)")
sleep(3.0)
...
while poco(text=target_string).exists():
    ...
    if elapsed_time >= 230 and not sent_60s_message:
#         send_message('AI風格化(img2img)\n等待時間超過120秒')
        sent_60s_message = True

    if elapsed_time >= one:
#         send_message('AI風格化(img2img)\n等待時間超過240秒,請手動測試目前狀況')
        stop_app(package=web)
        break
...
if poco(text="網絡錯誤，請再試一次。").exists():
    send_message('AI風格化(Comfy_ui_Cartoon pet)手機網路錯誤\n請QA手動測試目前狀況')
    print("開始第二次執行")
    auto_setup(__file__)
    poco = AndroidUiautomationPoco(use_airtest_input=True, screenshot_each_action=False)
    stop_app(package=web)
    ...
    if poco(text="編輯").exists():
        ...
        if poco(text="網絡錯誤").exists():
            send_message('AI風格化(Comfy_ui_Cartoon pet)第二次執行失敗\n錯誤提示:稍後再試試吧！\n請檢查測試機網路')
        ...
        start_time = time.time()
        sleep(3.0)
        print("開始第二次AI風格化(Comfy_ui_Cartoon pet)")
...
#                 send_message('AI風格化第二次執行\n等待時間超過120秒')
                sent_60s_message = True
            if elapsed_time >= two:
                send_message('【加急加急加急】\nAI風格化(Comfy_ui_Cartoon pet)第二次執行\n等待時間超過240秒,請QA手動測試目前狀況')
                stop_app(package=web)
                break
            time.sleep(1)
        ...
        if poco(text="網絡錯誤，請再試一次。").exists():
            send_message('【加急加急加急】\nAI風格化(Comfy_ui_Cartoon pet)第二次執行\手機網路錯誤,請QA手動測試目前狀況')

else:
    print(f"AI風格化(Comfy_ui_Cartoon pet)完成！回傳時間為：{duration:.2f}秒")
    
    
if duration > one:
    send_message('AI風格化(Comfy_ui_Cartoon pet)\n等待時間超過240秒')
    ...
    if poco(text="編輯").exists():
        ...
        if poco(text="網絡錯誤").exists():
            send_message('AI風格化(Comfy_uit)第二次執行失敗\n錯誤提示:稍後再試試吧！\n請檢查測試機網路')

        if poco(text="網絡錯誤，請再試一次。").exists():
            send_message('AI風格化(Comfy_ui)第二次執行失敗\n錯誤提示:無網路連線\n請檢查測試機網路')
...
#                 send_message('AI風格化第二次執行\n等待時間超過120秒')
        ...
        if poco(text="網絡錯誤，請再試一次。").exists():
            send_message('【加急加急加急】\nAI風格化(Comfy_ui)第二次執行\手機網路錯誤,請QA手動測試目前狀況')
