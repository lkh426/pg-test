...
# 等待目標字串出現
target_string = "com.photogrid.collage.videomaker:id/removeLoadingClose"  # 將此處替換為你要等待的字串
...
#             send_message(message_body)
    ...
    if poco(text="網絡錯誤").exists():
        # 構建訊息
        message_body = 'AI消除筆(Normal)第二次執行失敗\n錯誤提示:網路錯誤\n請檢查測試機網路'
        # 發送結果到飛書機器人群組
        send_message(message_body)
    if duration > two:
        print("第二次超過120秒")
